...
* ¿Cuál fue la situación en México entre 2020 y 2022? ¿Cuándo se tuvieron los primeros contagios? ¿Qué medidas se tomaron? ¿Cómo se establecieron los programas de prevención y vacunación? Estas fueron algunas de las cuestiones más relevantes.
...
* Casos confirmados y defunciones en Brasil (línea de tiempo sobre casos confirmados en Brasil).
...
temp_bz$acumulado_confirmado = cumsum(temp_bz$confirmed)
temp_bz$acumulado_muerte = cumsum(temp_bz$death)
...
temp_bz |> 
  ...
  labs(title = "Casos confirmados y defunciones en Brazil",
       x = "Fecha",
       y = "# de casos")
```

* Casos acumulados en Brazil

```{r defunciones_brazil}
# graficar
temp_bz |> 
  pivot_longer(cols = c(confirmed:acumulado_muerte),
               names_to = "Variable", values_to = "Cuenta") |> 
...
* Indicar valores de tasa de incidencia y mortalidad para Brazil

```{r tasa_incidencia_brazil}
